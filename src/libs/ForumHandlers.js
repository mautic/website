...
const handleForumTree = async (
  ...
  if (cacheRebuild) {
    ...
    topics = topics.map(topic => {
      topic.childReplies = replies.filter(reply => {
  ...
  const loadForumTopics = async (topics) => {
    ...
    let value_statements = topics.map((wptopic) => {
      ...
      return `(
                ${octotopic.id}, 
                ${sourceDataConn.escape(octotopic.subject)}, 
                ${sourceDataConn.escape(octotopic.slug)},
                ${octotopic.channelId}, 
                ...
                "${utils.formatMysqlTimestamp(octotopic.created)}", 
                "${utils.formatMysqlTimestamp(octotopic.created)}"                 
            )`;
    });
    let chunkInserts = utils.getArrayChunks(value_statements, 500).map((chunk) => {
  ...
  const loadForumReplies = async (replies) => {
    ...
    let value_statements = replies.map((wpreply) => {
      ...
      return `(
                ${octoreply.id},
                ${targetDataConn.escape(octoreply.subject)},
                ${targetDataConn.escape(
          octoreply.content
      )}, ${targetDataConn.escape(octoreply.content_html)}, 
  ...
  const polyfill_opContent = async (topics) => {
    let table = 'rainlab_forum_posts';
    let batchInsertStmt = `INSERT INTO ${table} (subject, content, content_html, topic_id, member_id, mtcorg_points, created_at, updated_at) VALUES `;

    let value_statements = topics.map(topic => {
      return `(
                ${targetDataConn.escape(topic.post_title)},
                ${targetDataConn.escape(topic.post_content)},
                ${targetDataConn.escape(topic.post_content)},
                ${targetDataConn.escape(topic.ID)},
                ${targetDataConn.escape(topic.post_author)},
                0, 
                "${utils.formatMysqlTimestamp(topic.post_date)}",
                "${utils.formatMysqlTimestamp(topic.post_date)}"
            )`;
    });
...
const handleForumMetrics = async () => {
  const targetDataConn = await config.getDbConnection(
  ...
  const loadTopicIds = async () => {
    ...
    let topicIds = await fetches.queryConnection(query, targetDataConn);
    ...
    return new Promise((resolve) => {
      let query_batchpostsintopics = `
                SELECT
                    ...
                    group by topics.id; `;
      targetDataConn.query(query_batchpostsintopics, (err, results) => {
        if (err) console.log(err);
        resolve(results);
      });
    });
  ...
  const updateTopicsFirstAndRecentPosts = async (patchArray) => {
    ...
    patchArray.forEach((row) => {
      let rowProm = new Promise((resolve) => {
        ...
        targetDataConn.query(q, (err, results) => {
          if (err) {
            console.log(err);
          }
