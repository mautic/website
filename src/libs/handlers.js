...
const handleForumTree = async (
    ...
    const loadForumTopics = async (topics) => {
        ...
        let value_statements = topics.map((wptopic) => {
            ...
            return `(
                ${octotopic.id}, 
                ${stagingConnection.escape(octotopic.subject)}, 
    ...
    const polyfill_opContent = async (topics) => {
        ...
        let value_statements = topics.map(topic => {
            return `(
                ${localdevConnection.escape(topic.post_title)},
                ${localdevConnection.escape(topic.post_content)},
                ${localdevConnection.escape(topic.post_content)},
                ${localdevConnection.escape(topic.ID)},
                ${localdevConnection.escape(topic.post_author)},
                0, 
